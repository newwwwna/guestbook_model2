...
<body>
	...
	<%
		GuestBookVO vo = (GuestBookVO) request.getAttribute("selectone");
	%>

	<table border='1px' cellspacing='1' cellpadding='2px' width='500'>
		<tr>
			<td>[<%=vo.getNo()%>] <input type="hidden" name="no" value="<%=vo.getNo()%>">
			</td>
			<td><%=vo.getName()%></td>
			...
			<th><form action="updatecon.jsp" method="post">
					<input type="hidden" name="no" value="<%=vo.getNo()%>"> <input
						type="hidden" name="name" value="<%=vo.getName()%>"> <input
						type="hidden" name="password" value="<%=vo.getPassword()%>">
